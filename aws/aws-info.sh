...
echo ">>> How many AWS services available?"
...
if [ "$OS_TYPE" == "macOS" ]; then  # it's on a Mac:
   ...
   echo ">>> From $OS_TYPE - $MONTH_FIRST_DAY to $MONTH_LAST_DAY "
...
#      --group-by Type=DIMENSION,Key=SERVICE | jq '.ResultsByTime[].Groups[] | select(.Metrics.UsageQuantity.Amount > 0) | .Keys[0]'
   ...
   echo ">>> What is each service costing me for the previous month:"
   aws ce get-cost-and-usage --time-period Start="$MONTH_FIRST_DAY",End="$MONTH_LAST_DAY" \
...
echo ">>> How do Snapshots breakdown by the volume used to create them?"
...
echo ">>> How much Data is in Each of my S3 Buckets?"
...
echo ">>> In dollars per month? (based on the standard tier price of $0.023 per GB per month):"
